...
// Chinese keywords
const CN_KEYWORDS: Record<string, number> = {
  '越狱': 25,
  '绕过': 15,
  '破解': 20,
  '黑客': 22,
...
const CONTEXT_INDICATORS = {
  length_anomaly: (text: string): number => {
    if (text.length < 5) return 3;
    if (text.length > 10000) return 15;
    if (text.length > 50000) return 25;
    return 0;
  },
  
  has_urls: (text: string): number => {
  ...
  has_special_chars: (text: string): number => {
    const specialChars = /[^\w\s.,!?;:'"()-]/g;
    const matches = (text.match(specialChars) || []).length;
    const density = matches / Math.max(text.length, 1);
    if (density > 0.15) return 20;
    if (density > 0.1) return 12;
    if (density > 0.05) return 6;
    return 0;
  },
  
  has_newline_anomaly: (text: string): number => {
...
export class SemanticAnalyzer {
  ...
  public assess(input: string): Threat[] {
  ...
  private checkKeywords(input: string): number {
    let score = 0;
  ...
  private checkChineseKeywords(input: string): number {
    let score = 0;
  ...
  private checkInjectionIntents(input: string): number {
    let score = 0;
  ...
  private checkStructures(input: string): number {
    let score = 0;
  ...
  private checkContext(input: string): number {
    let score = 0;
  ...
  public getRiskExplanation(input: string): {
    score: number;
    factors: string[];
    severity: string;
  } {
